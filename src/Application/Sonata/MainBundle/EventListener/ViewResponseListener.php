...
use FOS\RestBundle\EventListener\ViewResponseListener as BaseViewResponseListener;
 ...
 * @internal
 */
class ViewResponseListener extends BaseViewResponseListener implements EventSubscriberInterface
{
    private $viewHandler;
    private $forceView;
     ...
     */
    public function __construct(ViewHandlerInterface $viewHandler, $forceView, \Doctrine\Common\Cache\MemcachedCache $cacheDriver = null)
    {
    ...
    public function onKernelView(GetResponseForControllerResultEvent $event)
    {
        ...
        $canCacheResult = $request->isMethod('GET') && preg_match('/\/api\//im', $request->getRequestUri());
        ...
        if ($canCacheResult || $canRefreshCache) {
        ...
        if ($canCacheResult) {
            if ($this->cacheDriver->contains(md5($request->getRequestUri()))) {
                $response = $this->cacheDriver->fetch(md5($request->getRequestUri()));
        ...
        if ($canCacheResult) {
            $this->cacheDriver->save(md5($request->getRequestUri()), $response, 7200);
            $response->setSharedMaxAge(1800);
            $response->setPublic();
        }
